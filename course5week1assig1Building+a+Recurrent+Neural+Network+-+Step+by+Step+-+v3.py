...
def rnn_cell_backward(da_next, cache):
    ...
    dWax = np.dot((da_next*dtanh),np.transpose(xt))
    dxt = np.matmul(np.transpose(Wax),(dtanh*da_next))

    # compute the gradient with respect to Waa (≈2 lines)
    da_prev = np.matmul(np.transpose(Waa),(dtanh*da_next))
    dWaa = np.matmul(dtanh*da_next,np.transpose(a_prev))

    # compute the gradient with respect to b (≈1 line)
    dba =np.sum(dtanh*da_next,axis=1)
